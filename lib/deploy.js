...
var waterfallPipeDecorator = require('./util/waterfallDecorator');
...
var installBuild = function installBuild(data, callback) {
    if (path.basename(data.directoryPath) === 'public') {
        async.waterfall([
            npm.checkInstall,
            npm.runBuild
...
var deployProject = function(directory, done) {
    ...
    async.waterfall([
        ...
        waterfallPipeDecorator(checkSession),
        waterfallPipeDecorator(checkWidget),
        installBuild,
        pushProject
    ],
    function (error, results) {
